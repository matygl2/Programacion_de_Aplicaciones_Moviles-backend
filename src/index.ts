...
app.post("/api/personas", async (req, res) => {
  const { firstName, lastName, edad, arrivalDate, isWorking } = req.body;
  ...
  // edad también es obligatoria (la columna es NOT NULL en la base).
...
// Elimina definitivamente el registro de la base.
app.delete("/api/personas/:id", async (req, res) => {
  ...
  // 204 = éxito, sin contenido para devolver.
  res.status(204).send();
});
